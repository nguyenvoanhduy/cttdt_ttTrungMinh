...
export const PublicEventsPage = () => {
  ...
  const filteredEvents = MOCK_EVENTS.filter(event => {
      ...
      if (filter === 'Upcoming') return event.status === EventStatus.UPCOMING || event.status === EventStatus.ONGOING;
      if (filter === 'Past') return event.status === EventStatus.COMPLETED;
      return true;
  });
      ...
      {/* Banner */}
       <div className="bg-blue-900 py-16 text-center text-white relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative z-10">
          <h1 className="font-serif text-4xl font-bold mb-4">Sự Kiện & Lễ Hội</h1>
          <p className="text-blue-200 text-lg">Cập nhật các hoạt động sinh hoạt tôn giáo và xã hội</p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-6 md:px-12 py-12">
        ...
        <div className="flex justify-center mb-12">
            <div className="bg-gray-100 p-1 rounded-full inline-flex">
                {['Upcoming', 'Past', 'All'].map((f) => (
                    ...
                    >
                        {f === 'Upcoming' ? 'Sắp diễn ra' : f === 'Past' ? 'Đã diễn ra' : 'Tất cả'}
                    </button>
                ))}
            </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredEvents.length > 0 ? filteredEvents.map(event => (
                 <div key={event._id} className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 flex flex-col cursor-pointer" onClick={() => handleOpenEvent(event)}>
                    <div className="relative h-56 overflow-hidden">
                        ...
                        <div className="absolute top-0 right-0 p-4">
                            <span className={`px-3 py-1 rounded-full text-xs font-bold shadow-sm ${
                                event.status === EventStatus.UPCOMING ? 'bg-green-100 text-green-800' :
                                event.status === EventStatus.COMPLETED ? 'bg-gray-200 text-gray-800' :
                    ...
                    <div className="p-6 flex-1 flex flex-col">
                        <div className="flex gap-4 mb-4">
                            <div className="text-center min-w-[3.5rem] bg-blue-50 rounded-lg p-2 h-fit">
                                <span className="block text-xs font-bold text-gray-500 uppercase">{new Date(event.startTime).toLocaleString('default', { month: 'short' })}</span>
                                <span className="block text-2xl font-bold text-blue-600">{new Date(event.startTime).getDate()}</span>
                            </div>
                            <div>
                                <div className="text-xs font-bold text-amber-600 uppercase mb-1">{event.eventType}</div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
              ...
              <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto flex flex-col animate-in zoom-in-95 duration-300">
                  ...
                  <div className="relative h-64 md:h-80 flex-shrink-0">
                      <img 
                          src={selectedEvent.bannerUrl || `https://picsum.photos/seed/${selectedEvent._id}/800/600`} 
                      ...
                      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-8">
                          ...
                          <div className="flex flex-wrap items-center gap-4 text-gray-200 text-sm">
                               <div className="flex items-center">
                                   ...
                                   {new Date(selectedEvent.startTime).toLocaleDateString('vi-VN', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                               </div>
                               <div className="flex items-center">
                      ...
                      <div className="flex flex-col md:flex-row gap-12">
                          <div className="flex-1">
                              ...
                              <div className="prose prose-blue text-gray-600 mb-8">
                                  <p>{selectedEvent.description || "Chưa có mô tả chi tiết cho sự kiện này. Vui lòng liên hệ Ban Tổ Chức để biết thêm thông tin."}</p>
                                  <p>Đến với sự kiện, quý đạo hữu sẽ được tham gia vào các hoạt động ý nghĩa, lắng nghe thuyết giảng giáo lý và cùng nhau cầu nguyện cho thế giới hòa bình, vạn linh sanh chúng được an lạc.</p>
                              </div>

                              <h3 className="text-lg font-bold text-gray-900 mb-4">Nội dung chương trình</h3>
                              <ul className="space-y-4 mb-8">
                                  <li className="flex gap-4">
                                      ...
                                      <div className="pb-4 border-l-2 border-blue-100 pl-6 relative">
                                          ...
                                          <p className="font-bold text-gray-800">Khai mạc & Tiếp đón</p>
                                          <p className="text-sm text-gray-500 mt-1">Đón tiếp quý đại biểu và đạo hữu.</p>
                                      </div>
                                  </li>
                                  <li className="flex gap-4">
                                      <div className="w-20 font-bold text-blue-600 text-sm text-right shrink-0">
                                          {new Date(new Date(selectedEvent.startTime).getTime() + 60*60*1000).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                                      </div>
                                      <div className="pb-4 border-l-2 border-blue-100 pl-6 relative">
                                          <div className="absolute -left-[5px] top-1.5 w-2 h-2 rounded-full bg-blue-500"></div>
                                          <p className="font-bold text-gray-800">Phần Lễ chính thức</p>
                                          <p className="text-sm text-gray-500 mt-1">Dâng hương và cầu nguyện.</p>
                                      </div>
                                  </li>
                              </ul>
                          </div>
                          ...
                          <div className="md:w-72 shrink-0">
                              <div className="bg-gray-50 p-6 rounded-xl border border-gray-100 sticky top-4">
                                  ...
                                  {(() => {
                                      const organizer = MOCK_PERSONALS.find(p => p._id === selectedEvent.organizerId);
                                      return organizer ? (
                                          <div className="flex items-center mb-6">
                                              <img src={organizer.avatarUrl} alt="" className="w-10 h-10 rounded-full mr-3" />
                                              <div>
                                                  <p className="text-sm font-bold text-gray-900">{organizer.fullname}</p>
                                                  <p className="text-xs text-gray-500">{organizer.position}</p>
                                              </div>
                                          </div>
                                      ) : null;
                                  })()}
                                  ...
                                  <div className="space-y-3 mb-6">
                                      <div className="flex justify-between text-sm">
                                          ...
                                          <span className="font-medium text-green-600">{selectedEvent.status}</span>
                                      </div>
                                      <div className="flex justify-between text-sm">
                                          <span className="text-gray-500">Số lượng:</span>
                                          <span className="font-medium">{selectedEvent.participantsCount} người</span>
                                      </div>
                                  </div>
                                  ...
                                  {!registerSuccess ? (
                                      <button 
                                        ...
                                        disabled={isRegistering || selectedEvent.status === EventStatus.COMPLETED}
                                      ...
                                      <div className="bg-green-100 text-green-700 p-4 rounded-lg text-center animate-in zoom-in">
                                          ...
                                          <p className="text-xs mt-1">BTC sẽ gửi thông tin chi tiết qua email.</p>
                                      </div>
                                  )}
                              </div>
                          </div>
