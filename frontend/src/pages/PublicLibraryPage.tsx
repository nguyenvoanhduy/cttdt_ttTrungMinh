...
export const PublicLibraryPage = () => {
  ...
  const activeTabType =
    LIBRARY_TABS.find((t) => t.id === activeTabId)?.type || "book";
      ...
      <div className="bg-blue-900 py-16 text-center text-white relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative z-10">
          ...
          <p className="text-blue-200 text-lg">
            Kho tàng tài liệu giáo lý và âm nhạc Đạo
          </p>
        </div>
      </div>
                    ...
                    <div className="mt-auto">
                      <div className="flex flex-wrap gap-1 mb-3">
                        {book.categories.map((c) => (
                          <span
                            key={c}
                            className="px-1.5 py-0.5 bg-gray-100 text-gray-500 rounded text-[10px] uppercase font-bold tracking-wide"
                          >
                            {c}
                          </span>
                        ))}
            ...
            <div className="flex-1 bg-gray-100 overflow-y-auto p-8 flex justify-center">
              <div className="bg-white shadow-xl w-full max-w-3xl min-h-full p-12 text-gray-800">
                ...
                <div className="space-y-4 text-justify leading-relaxed font-serif">
                  <p>
                    <strong>LỜI NÓI ĐẦU</strong>
                  </p>
                  <p>
                    Đại Đạo Tam Kỳ Phổ Độ là một nền tôn giáo lớn, có tôn chỉ,
                    mục đích rõ ràng, nhằm quy nguyên tam giáo, hiệp nhất ngũ
                    chi, phục nhất nhân tâm, tạo thế giới đại đồng.
                  </p>
                  <p>
                    Tài liệu này được biên soạn nhằm giúp quý đạo hữu và độc giả
                    gần xa hiểu rõ hơn về những giáo lý căn bản, những quy điều
                    và luật pháp của Đạo, từ đó tu tâm dưỡng tánh, sống tốt đời
                    đẹp đạo.
                  </p>
                  <p className="text-gray-400 italic text-center py-12">
                    [Nội dung tài liệu đang được tải...]
                  </p>
                  <p>
                    (Đây là bản xem thử mô phỏng. Trong thực tế, phần này sẽ
                    hiển thị nội dung PDF hoặc DOCX đầy đủ của tài liệu.)
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 w-full text-center md:text-left text-white">
              ...
              <div className="mb-8 group">
                ...
                <div className="flex justify-between text-xs text-gray-400 mt-2 font-mono">
                  <span>
                    {Math.floor(((progress / 100) * playingSong.duration) / 60)}
                    :
                    {String(
                      Math.floor(((progress / 100) * playingSong.duration) % 60)
                    ).padStart(2, "0")}
                  ...
                  <span>
                    {Math.floor(playingSong.duration / 60)}:
                    {String(playingSong.duration % 60).padStart(2, "0")}
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-center md:justify-start gap-8">
                ...
                <button className="text-gray-200 hover:text-white transition-colors">
                  <Icons.SkipBack className="w-8 h-8 fill-current" />
                </button>
                ...
                >
                  {isPlaying ? (
                    <Icons.Pause className="w-6 h-6 fill-current" />
                  ) : (
                    <Icons.Play className="w-6 h-6 fill-current ml-1" />
                  )}
                </button>

                <button className="text-gray-200 hover:text-white transition-colors">
                  <Icons.SkipForward className="w-8 h-8 fill-current" />
                </button>
                <button className="text-gray-400 hover:text-white transition-colors">
